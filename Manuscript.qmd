...
rm("cen_data_2011_1")
load(file="data-prep/data-products/cen_data_2016_hamiltononly.Rdata")
...
ham_city_bound <- st_read("data-prep/data-inputs/Boundaries/City_Boundary.shp") |> st_transform(st_crs(cen_data_2011_hamiltononly))
ham_community_bounds <- st_read("data-prep/data-inputs/Boundaries/Community_Boundaries.shp") |> st_transform(st_crs(cen_data_2011_hamiltononly))
```
...
load(file="data-prep/data-products/TTS_gamma2011_motor.Rdata")
load(file="data-prep/data-products/TTS_gamma2016_motor.Rdata")
load(file="data-prep/data-products/TTS_exp2011_nonmotor.Rdata")
load(file="data-prep/data-products/TTS_exp2016_nonmotor.Rdata")
```
...
tmap_save(catchment_schools_plot, "Figures/catchment_schools_plot.png", height=7, width=12, dpi=600)
```

```{r,include=FALSE}
load(file="data-prep/data-products/avail_2011.RData")
load(file="data-prep/data-products/avail_2016.RData")
```
...
tmap_save(stud_pop_rate_LIM_plot, "Figures/stud_pop_rate_LIM_plot.png", height=7, width=12, dpi=600)
```

```{r importing-taz-and-associated-flows}
#Read the boundaries of the traffic analysis zones (retrieved from the TTS 2016):
ggh_taz <- st_read("data-prep/data-inputs/TTS/tts06_83_region.shp") %>% st_transform(crs = 32617)
...
#Read the boundaries of the traffic analysis zones (retrieved from the TTS 2016):
ggh_pd <- st_read("data-prep/data-inputs/TTS/tts06_pd_83.shp") %>%
...
load(file="data-prep/data-inputs/TTS/od_trips_perc_2011.Rdata")
load(file="data-prep/data-inputs/TTS/od_trips_perc_2016.Rdata")
...
tmap_save(OD_flow_plot, "Figures/OD_flow_plot.png", height=7, width=12, dpi=600)
```
...
ggsave("Figures/TLD_empirical_wtheo_curve_plot.png", height=7, width=12, dpi=600)
```
...
tmap_save(Vim_vvim_20112016_plot, "Figures/Vim_vvim_20112016_plot.png", height=7, width=12, dpi=600)
```
...
tmap_save(Vim_vvim_20112016_CHANGE_plot, "Figures/Vim_vvim_20112016_CHANGE_plot.png", height=7, width=12, dpi=600)
```
...
walk_OD_flow <- tm_shape(od_trips_perc_walk |> mutate(mode = "non-motorized"), bbox=SAvail_i_DA_extra) +
  ...
  tm_add_legend(type = "symbol",
                ...
                labels = "TAZ centroid")
OD_flow_plot <- tmap_arrange(car_OD_flow_plot, walk_OD_flow, nrow = 2)
tmap_save(OD_flow_plot, "Figures/SA-and-SAperStud-20112016.png", height=7, width=12, dpi=600)
```
...
```{r catchments-2011-2016, fig.cap="\\label{fig:Fig1}The school catchments, estimated on the ground capacities of schools summed per catchment, and school status for both public and public catholic school boards for year 2011 and 2016. OTGC is presented by quartiles. ", out.width="100%", echo=FALSE}

knitr::include_graphics("Figures/catchment_schools_plot.png")
```
...
```{r elem-res-rate-2016, fig.cap="\\label{fig:Fig2}The magnitude (top row) and rate per residential unit (middle row) of elementary aged student population per DA in 2011 and 2016. The proportion of low-income households with children under 18 years old is also shown (bottom row). All scales are represented in quartiles.", out.width="100%", echo=FALSE}
knitr::include_graphics("Figures/stud_pop_rate_LIM_plot.png")
```
...
```{r imped-20112016, fig.cap="\\label{fig:Fig3}The origin-destination flows from home-based motorized and non-motorized school trips for students between 5-13 years old as retrieved from TTS 2011 and 2016 mapped atop proportion of TAZ non/motorized modal share, DA boundaries, and community boundaries. ", out.width="100%", echo=FALSE}
knitr::include_graphics("Figures/OD_flow_plot.png")
```
...
```{r imped-20112016-2, fig.cap="\\label{fig:Fig4}The empirical and theoretical motorized and non-motorized impedance functions. ", out.width="100%", echo=FALSE}
knitr::include_graphics("Figures/TLD_empirical_wtheo_curve_plot.png")
```
...
```{r Vim-vim-4-plots, fig.cap="\\label{fig:Fig6}Multimodal spatial availability (greys) and spatial availability per mode-using student (diverging reds and greens) aggregated at the level of DA for 2011 and 2016.", out.width="100%", echo=FALSE}
knitr::include_graphics("Figures/Vim_vvim_20112016_plot.png")
```
...
```{r change-Vim-vim-plot, fig.cap="\\label{fig:Fig7}The change in multimodal spatial availability and spatial availability per mode-using student between 2016 and 2011 at the level of DA of the 2016 Canadian Census.", out.width="100%", echo=FALSE}
knitr::include_graphics("Figures/Vim_vvim_20112016_CHANGE_plot.png")
```
