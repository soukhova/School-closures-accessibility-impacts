---
title: "03-TTS-travel-to-school-impedance"
format: html
---
Load packages:
```{r load-packages}
...
```{r}
ggh_taz <- st_read("data-inputs/TTS/tts06_83_region.shp") %>%   st_transform(crs = 32617)
```
...
```{r} 
#load catchments - available through the hamONdests package (https://github.com/soukhova/hamONdests):
library(hamONdests)
data(School_Catchments_2011_2016)

CATCH_2011_Public <- School_Catchments_2011_2016%>%filter(Year == 2011 & Level == "Elementary" & System == "Public") %>% st_set_crs(st_crs(ham_taz))
CATCH_2011_Cath <- School_Catchments_2011_2016%>%filter(Year == 2011 & Level == "Elementary" & System == "Catholic")%>% st_set_crs(st_crs(ham_taz))
CATCH_2016_Public <- School_Catchments_2011_2016%>%filter(Year == 2016 & Level == "Elementary" & System == "Public")%>% st_set_crs(st_crs(ham_taz))
CATCH_2016_Cath <- School_Catchments_2011_2016%>%filter(Year == 2016 & Level == "Elementary" & System == "Catholic")%>% st_set_crs(st_crs(ham_taz))
...
```{r}
## THIS FUNCTION WAS CREATED USING CHATGDPT on DEC 6 2023 -- this is a function to check if values in a row of df1 are within any list in df2
...
save("output_OD_car", file = "data-inputs/TTS/TTS_tt_car.Rdata")
...
save("output_OD_walk", file = "data-inputs/TTS/TTS_tt_walk.Rdata")
```
...
```{r}
load(file = "data-inputs/TTS/TTS_tt_car.Rdata")
TTS_tt_car <- output_OD_car
rm("output_OD_car")
summary(TTS_tt_car)
#we'll set 0 travel times to equal 7 min for inter-zonal travel, half of the 1st Qu. travel time.

load(file = "data-inputs/TTS/TTS_tt_walk.Rdata")
TTS_tt_walk <- output_OD_walk
rm("output_OD_walk")
summary(TTS_tt_walk) #max trip length calculated is 27 mins -- this corresponds to 'within catchment'. lets make 0 min trips into half of 1st Q. so 4min.
```
...
od_trips_2011_motor <- od_trips_2011_motor %>% 
  merge(TTS_tt_car, by.x = c("Origin", "Destination"), by.y = c("from_id", "to_id")) %>%
...
od_trips_2011_nonmotor <- od_trips_2011_nonmotor %>%
  merge(TTS_tt_walk, by.x = c("Origin", "Destination"), by.y = c("from_id", "to_id")) %>%
  merge(ham_taz %>% st_drop_geometry(), by.x="Origin", by.y="GTA06") 
...
tld_2011_nonmotor <- od_trips_2011_nonmotor |> filter(travel_time <= 27) |>
  mutate(tt_classes = cut(travel_time, 
                          150,
                          ordered_result = TRUE)) %>%
  group_by(tt_classes) %>%
...
tld_2016_nonmotor <- od_trips_2016_nonmotor |> filter(travel_time <= 27) |>
  mutate(tt_classes = cut(travel_time, 
                          150,
                          ordered_result = TRUE)) %>%
  group_by(tt_classes) %>%
...
all_tt_2011_nonmotor <- od_trips_2011_nonmotor |> filter(travel_time <= 27) |>
  dplyr::select(Trips, travel_time)
...
all_tt_2016_nonmotor <- od_trips_2016_nonmotor  |> filter(travel_time <= 27) |>
  dplyr::select(Trips, travel_time)
...
```{r}
save(gamma2011_motor, file="data-products/TTS_gamma2011_motor.Rdata")
save(gamma2016_motor, file="data-products/TTS_gamma2016_motor.Rdata")
save(exp2011_nonmotor, file="data-products/TTS_exp2011_nonmotor.Rdata")
save(exp2016_nonmotor, file="data-products/TTS_exp2016_nonmotor.Rdata")
```
