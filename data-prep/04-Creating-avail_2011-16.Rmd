...
OD_2011 <- RED_LU_centroids_2011 %>% 
  cross_join(SCHOOLS %>% st_drop_geometry() |> filter(Year != 2016) %>% dplyr::select(c("SchoolID", "OTGC2011", "GTA06_Dest")))
...
```{r}
#reduce dimensions
OD_2011 <- OD_2011 %>% dplyr::select(-c("LUC1_DESC", "LUC2_DESC", "DA_dwellings", "CMA_UID", "Region Name", "geometry","Area (sq km)"))
...
# 2016 calcs
```{r}
rm(avail_2011,ttm_car_2011, ttm_walk_2011, od_trips_perc_2011, od_trips_perc_comm_2011,RED_LU_centroids_2011)
```
...
OD_2016 <- RED_LU_centroids_2016 %>% 
  cross_join(SCHOOLS %>% st_drop_geometry() |> filter(Year != 2011) %>% dplyr::select(c("SchoolID", "OTGC2016", "GTA06_Dest")))
...
```{r}
#checks!, these should be the total number of kids (^the code above) -- 58865
OD_2016$kid_pop %>% sum()
